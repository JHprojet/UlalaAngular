...
import { Classe, Image, Strategy, CharactersConfiguration, User, Team, 
         BossesPerZone } from '../../models';

import { CharactersConfigurationService, StrategyService, ImageService, 
         BossesPerZoneService, ClasseService, AccessService, CustomValidators } from '../../services';
...
export class AddStrategieComponent implements OnInit {
  ...
  constructor(private translate:TranslateService, private ccService:CharactersConfigurationService, private imageService:ImageService,
            private cd:ChangeDetectorRef, private classeService:ClasseService, private bzService:BossesPerZoneService,
            private accessService:AccessService, private v:CustomValidators, private stratService:StrategyService,
  ...
  selectMyTeams:Team[] = new Array<Team>();
  ...
  ngOnInit(): void {
    ...
    if(this.accessService.getSession("Info")) {
      ...
      if(this.accessService.getSession("Teams").length) {
        ...
        this.selectMyTeams = new Array<Team>();
      }
    }
  ...
  onSubmitWithTeam() {
    ...
    if(this.accessService.getSession("Info")) { E.User.Id = this.accessService.getSession("Info").Id }
    //Upload 4 Images
    for (let item of this.Images) { this.uploadFile(item); }
    //Feeling paths of the images to the strategy
    E.ImagePath1 = this.FileNames[0];
    E.ImagePath2 = this.FileNames[1];
    E.ImagePath3 = this.FileNames[2];
    E.ImagePath4 = this.FileNames[3];
    //Post strategy
    this.stratService.postStrategy(E).subscribe(result => { 
      //If success, reset form + input file and display success message (5s)
      this.AddFormWithTeam.reset();
      this.OkImage$.next(false);
      this.AddFormWithTeam.controls.Boss.disable();
      let ToClean = document.getElementById("inputImagesWithTeam") as HTMLInputElement;
      ToClean.value = "";
      this.UploadOK = true;
